...
export class ListarOcorrenciaComponent implements OnInit {
  ...
  applyFilter(event: Event) {
    ...
    this.FILTRAR_DADOS = this.OCORRENCIA_INFO.filter((element) => {
      return (
        element.id.toString().includes(filterValue) ||
        element.titulo.toLowerCase().includes(filterValue) ||
        element.usuario.toLowerCase().includes(filterValue) ||
        (element.servidor?.nome?.toLowerCase && element.servidor.nome.toLowerCase().includes(filterValue)) ||
        element.dataAbertura.toLowerCase().includes(filterValue) ||
        element.prioridade.toLowerCase().includes(filterValue) ||
        this.status(element.status).toLowerCase().includes(filterValue)
      );
    });
  }
